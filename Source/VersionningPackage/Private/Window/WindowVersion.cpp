...
void SWindowVersion::Construct(const FArguments& InArgs)
{
    ...
    majorField = SNew(SEditableTextBox).IsReadOnly(true)
...
FReply SWindowVersion::SetVersion()
{
    ...
    OPEN_SUCCESS_POPUP("Version has been set to " + FString::FromInt(major) + "." + FString::FromInt(minor) + "." + FString::FromInt(patch) + ".");
    return FReply::Handled();
}

TSharedRef<SVerticalBox> SWindowVersion::CreateVersionField(const FString& Label, const TSharedPtr<SWidget>& FieldWidget)
{
